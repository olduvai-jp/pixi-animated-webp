...
{
    // Note: This test requires a `example.webp` file in `test/resources`.
    // You can create one by converting the existing `example.gif`.
    const arrayBuffer = toArrayBuffer(
        fs.readFileSync(path.join(__dirname, './resources/example.webp'))
    );
    ...
    describe('fromBuffer()', () =>
    {
        ...
        {
            const animation = await AnimatedWebP.fromBuffer(arrayBuffer);
    ...
    describe('currentFrame', () =>
    {
        ...
        {
            const animation = await AnimatedWebP.fromBuffer(arrayBuffer, { autoPlay: false });
