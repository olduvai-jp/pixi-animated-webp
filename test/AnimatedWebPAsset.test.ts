...
import { AnimatedWebP, AnimatedWebPAsset } from '../src';
import { createServer } from './resources';

describe('AnimatedWebPLoader', () =>
{
    let server: any;
    let baseUrl: string;
    ...
    {
        expect(typeof AnimatedWebPAsset).toBe('object');
    });

    it('should load a webp file', async () =>
    {
        const url = `${baseUrl}/example.webp`;

        Assets.add({ alias: 'test', src: url });
        const test = await Assets.load('test');

        expect(test);
        expect(test).toBeInstanceOf(AnimatedWebP);
    ...
    it('should load a webp file with options', async () =>
    {
        const url = `${baseUrl}/example.webp`;
